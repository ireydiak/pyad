...
class MemoryUnit(nn.Module):
    ...
    *** Code largely inspired by the original repository for MemAE (https://github.com/donggong1/memae-anomaly-detection/) ***
    """
    def __init__(self, mem_dim: int, in_features: int, shrink_thres=0.0025, device=None):
        super(MemoryUnit, self).__init__()
        self.device = device or "cuda" if torch.cuda.is_available() else "cpu"
        self.mem_dim = mem_dim
        self.in_features = in_features
        # M x C
        self.weight = Parameter(torch.Tensor(self.mem_dim, self.in_features)).to(self.device)
    ...
    def forward(self, x: torch.Tensor) -> (float, float):
