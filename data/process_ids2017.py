...
def merge_step(path_to_files: str, export_path: str = None) -> Tuple[pd.DataFrame, dict]:
    stats = defaultdict()
    df = pd.DataFrame()
    ...
    for f in os.listdir(path_to_files):
        ...
        print(f)
    if export_path:
        df.to_csv(os.path.join(export_path, "merged.csv"))
    stats["dropped_cols"] = ""
